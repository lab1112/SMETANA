...
int main(void)
{
  //переменные
  uint32_t tmpreg;
  int16_t  massShIM[6] = {0,0,0,0,0,0};
  int16_t  massData[4];
  uint16_t snd = 0;
  uint16_t clr = 0;	
  uint16_t ReturnData[1][15];
  float*   dataSetUK;
  float*   dataGetUK;
	
  int   i = 0, j = 0, k = 0;//1 SSP send cycle counter,2 SSP send cycle counter,SSP recieve cycle counter
  int   a = 0, b = 0;
  float x = 0, y = 0;//требуемый угол поворота по горизонтали,требуемый угол поворота по вертикали
  float x1= 0, y1= 0;
  float da= 0, db= 0;//рассогласование угла по горизонтали,рассогласование угла по вертикали
  float dda=0, ddb=0;//производная рассогласования по горизонтали,производная рассогласования по вертикали
  float ia =0, ib =0;//интеграл рассогласования по горизонтали,интеграл рассогласования по вертикали
  float pa =0, pb =0;//управляющий сигнал 1,управляющий сигнал 2
  float da1=0,db1 =0;
  float g = 0,v   =0;//текущий угол поворота по горизонтали в градусах,текущий угол поворота по вертикали в градусах
  float g1= 0,v1 = 0;//текущий угол поворота по горизонтали с датчика,текущий угол поворота по вертикали с датчика
  ...
  //конфигурация порта MIL_STD(XP22)
  ...
  //инициализация преобразователя сигналов датчика
  for(j = 0 ; j < 11 ; j++)
  {
    for(i = 0 ; i < 2 ; i++)
    {		
  ...
  for(k = 0 ; k < 13 ; k++)
  {
    ...
    ReturnData[0][k] = SSP_ReceiveData(MDR_SSP0);	
  }
  ...
  while (1)
  {
    //считывание требуемых значений углов
